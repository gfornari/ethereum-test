...
def retrieve_enodes(conf):
    all_enodes = {}
    host_number = len(conf["nodes"])
    print("there are " + str(host_number) + " hosts")
    for host in range(0, host_number):
        client_number = conf["nodes"][host]["client_number"]
        if client_number > 0:
            enodes = [0] * client_number
            for i in range(0, client_number):
                rpc_port = "%d%02d " % (PREFIX_RPC_PORT, i)
                cmd = "geth  attach http://" + \
                conf["nodes"][host]["address"] + ":" + rpc_port + \
                " --exec admin.nodeInfo.enode"
                enodes[i] = subprocess.check_output(cmd.split())
                enodes[i] = enodes[i].decode('ascii').split()[0].\
                replace("[::]", conf["nodes"][host]["address"])
                # ~ print(enodes[i])
...
def add_peers(conf, enodes):
    ...
    for host in range(0, host_number):
        client_number = conf["nodes"][host]["client_number"]
        if client_number > 0:
            for other_host in range(0, host_number):
                    ...
                    for i in range(0, client_number):
                        for j in range(0, other_client_number):
                                if not (i == j and host == other_host):
                                    ...
                                    cmd = "geth attach http://" + \
                                    conf["nodes"][host]["address"] + ":" + rpc_port +\
                                     " --exec admin.addPeer(" + enodes[str(other_host)][j] + ")"
...
def check_number_of_peers(conf):
    ...
    for host in range(0, host_number):
        ...
        for i in range(0, client_number):
            rpc_port = "%d%02d " % (PREFIX_RPC_PORT, i)
            cmd = "geth attach http://" + conf["nodes"][host]["address"]\
            + ":" + rpc_port + " --exec net.peerCount"
